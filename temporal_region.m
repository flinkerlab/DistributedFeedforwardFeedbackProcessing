...
% plot_data_all_T1 = get_plot_data(paths, 'iij', 'lh');
attrs={'spec'};
%% 
elec_range = 1:128;
...
for ir=1:3
    ...
    [frames,err,tticks] = attr_vis_tau('spec',post,visdiff,postabs,max_att,plot_on_same_brain,above_median,temp,wreg,entire_period,true,invest_regions{ir},reference{ir},dataname{ir},is_causal{ir});
...
%     for x = 1:size(frames,2); fill([tticks,fliplr(tticks)] , [smoothcurve(:,x,:)'-smootherr(:,x,:)',fliplr(smoothcurve(:,x,:)'+smootherr(:,x,:)')],'FaceAlpha',0.3,'EdgeAlpha',0); end
    hold on; plot(tticks,0.19*ones(size(tticks)),'LineWidth',4,'Color',[0.5,0.5,0.5,0.7],'LineStyle',':');
    ax=gca;ax.FontSize=20;  set(gca,'linewidth',2)
%             figure(fig1);p=plot(frames(end:-1:1,:,:),'LineWidth',3);
    ylim(haxis(sub2ind([rows,cols],dumm)),[ylims_min(1),ylims_max(1)]);
    set(haxis(sub2ind([rows,cols],dumm)),'box','off','TickDir','out');
    xlim(haxis(sub2ind([rows,cols],dumm)),[tticks(1),tticks(end)]);
    pbaspect([1.5 1 1]);
    legend(invest_regions{ir});
...
function [frames,err,tticks] = PlotAtt(Att1_cell, mask_cell,coord_cell,regions,causal,brain,colormap,Att_c,Att_cpre,Att_a,ref,investreg)
    ...
    %             Att_ref = Att_c;
        ...
        for ii = 1:length(Att1_cell)
            att = cat(3,att,Att1_cell{ii});
            att_ref = cat(3,att_ref,Att_ref{ii});
            mask = cat(1,mask,mask_cell{ii});
            region = cat(1,region,regions{ii});
            coord = cat(2,coord,coord_cell{ii});
        end
        col=144;%13;
        offset=16;
        rate = 8;
...
%                  'VariableNames', {'iiject'});
%     Subj.iiject = paths.Subj;
...
function [frames,err,tticks] = attr_vis_tau(attr,post,visdiff,postabs,max_att,plot_on_same_brain,above_median,temp,wreg,entire_period,return_curve,investreg,reference,dataname,is_causal)
    % choose a color map and set indecies of colors for each electrode
    color_map = hot(256);%afmhot;%hot;
    color_map = color_map(end:-1:1,:);
...
%     Ss = {'717','742','749'};
    ...
    for ii=1:SUB
        ...
        load(['fig45/NY_',Ss{ii},'_elec_entiregrid.mat']);
        region_cell{ii} = regions;
        load(['fig45/entiregrid_',Ss{ii},'_normxdim_',dataname,'_covnorm_NOVISWEIGHT_step1/occ_covnorm_temporal_tstep1_tau_8.mat',]);
        load(['fig45/contrib_ref_',Ss{ii},'.mat']);
        color_map = hot(256);%afmhot;%hot;
        color_map = color_map(end:-1:1,:);
        clrmap = color_map;
        eval([attr,'=',attr,';']);
        ...
        att_cell_c{ii} = Att1_c;
        att_cell_cpre{ii} = Att1_cpre;
        att_cell_a{ii} = Att1_a;
        ...
        if plot_on_same_brain 
            if ii == SUB
                if ~return_curve
                    ...
                    [frames,err,tticks] = PlotAtt(Att1_cell, mask_cell,coord_cell,region_cell,causal,plot_data_all_mni,clrmap,att_cell_c,att_cell_cpre,att_cell_a,ref,investreg);
                end
            end
        else
